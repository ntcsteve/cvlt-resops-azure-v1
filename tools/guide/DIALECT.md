...
The five figure SVGs are GENERATED by `tools/guide/figures/make_*.py`,
which mirror their colors from `assets/tokens.css` and embed the official
icons as data URIs. Edit the generator and re-run it; never hand-edit an
SVG. Regeneration is deterministic, so an edited SVG with an untouched
generator shows up as a diff the next time anyone runs the script.
