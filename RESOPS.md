...
**There is no way to know a backup is good without opening it and looking.** The vendor's scan does open it, and looks for what *it* recognizes. Only your own check knows whether *your* service still works. That is why you want both, and why almost nobody has either.

So the attester is [`verify.sh`](VERIFY.md): the restore drill opens the recovery point in isolation and runs the workload's *own* check inside the restored copy. The one line it prints, `OK:` or `FAIL:`, is the attestation. About seventy lines of shell anyone in the room can read.
...
- **The compliance crosswalk is indicative.** It supports a resilience program. It is not a formal attestation.
- **Nobody owns this by default.** The gate belongs to CI, `verify.sh` to the app team, the drill to platform, the policy to risk. In most organizations that spans three teams who do not currently talk about recovery together. The org problem is usually larger than the technical one.
